            ...
            <div className="card recent-order-card">
              ...
              <div style={{ display: 'grid', gap: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                  ...
                  <span className={`status-${latestOrder.status.toLowerCase()}`}>
                    {getStatusMeta(latestOrder.status).label}
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                  ...
                  <span style={{ fontWeight: 800 }}>
                    Rs.{Number(latestOrder.totalPrice || 0).toFixed(2)}
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                  ...
                  <span
                    style={{
                      fontWeight: 700,
                      color: latestOrder.paymentStatus === 'received' ? 'var(--green)' : '#8a5a00',
                    }}
                  >
                    {getPaymentStatusMeta(latestOrder.paymentStatus).label}
                  </span>
                </div>
              </div>
              <div
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginTop: '18px' }}>
                <Link
                  to={`/orders/${encodeURIComponent(latestOrder.id)}`}
                ...
                <Link
                  to={`/track?ref=${encodeURIComponent(getOrderReference(latestOrder))}`}
      ...
      <section className="why-us">
        ...
        <div className="usp-grid why-us-grid">
          <article className="usp-card">
            <div className="usp-emoji">SAFE</div>
            <h3>Verified stock</h3>
            <p>
              We inspect every batch, update inventory continuously, and only dispatch medicines that pass our strict
              quality checklist.
            </p>
          </article>
          <article className="usp-card">
            <div className="usp-emoji">FAST</div>
            <h3>Doorstep speed</h3>
            <p>
              Orders placed before 6PM go out the same day inside Attrasand; regional villages see delivery within 24
              hours.
            </p>
          </article>
          <article className="usp-card">
            <div className="usp-emoji">VALUE</div>
            <h3>Transparent pricing</h3>
            <p>
              MRP rates, discounts, and delivery charges are clearly shown before you confirm the order, with no
              surprises.
            </p>
          </article>
        </div>
